...
func (b iconAndTextButton) Layout(gtx layout.Context) layout.Dimensions {
	return material.ButtonLayout(b.theme, b.button).Layout(gtx, func(gtx Ctx) D {
		iconAndLabel := layout.Flex{Axis: layout.Horizontal, Alignment: layout.Middle}
		textIconSpacer := unit.Dp(5)

		layIcon := layout.Rigid(func(gtx Ctx) D {
			return layout.Inset{Right: textIconSpacer}.Layout(gtx, func(gtx Ctx) D {
				var d D
				if icon != nil {
		...
		layLabel := layout.Rigid(func(gtx Ctx) D {
			return layout.Inset{Left: textIconSpacer}.Layout(gtx, func(gtx Ctx) D {
	...
	widgets := []layout.Widget{
		...
		func(gtx Ctx) D {
			...
			return layout.Flex{Alignment: layout.Middle}.Layout(gtx,
				...
				layout.Rigid(func(gtx Ctx) D {
					return in.Layout(gtx, func(gtx Ctx) D {
						for button.Clicked() {
							green = !green
						}
				...
				layout.Rigid(func(gtx Ctx) D {
					return in.Layout(gtx, func(gtx Ctx) D {
						return material.Clickable(gtx, flatBtn, func(gtx Ctx) D {
							return layout.UniformInset(unit.Dp(12)).Layout(gtx, func(gtx Ctx) D {
		...
		func(gtx Ctx) D {
			return layout.Flex{Alignment: layout.Middle}.Layout(gtx,
				...
				layout.Rigid(func(gtx Ctx) D {
					return layout.Inset{Left: unit.Dp(16)}.Layout(gtx, func(gtx Ctx) D {
						text := "enabled"
						if !swtch.Value {
							text = "disabled"
							gtx = gtx.Disabled()
...
func fontCollection() *text.Collection {
