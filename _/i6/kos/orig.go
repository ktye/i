...
func fpack64(sign, mant uint64, exp int, trunc uint64) uint64 {
	//fmt.Println(" fpak64", sign, mant, exp, trunc)
...
func fadd64(f, g uint64) uint64 {
	...
	trunc := gm & (1<<shift - 1)
	gm >>= shift
	//fmt.Println(" shift/trunc", shift, trunc, fm, gm)
	if fs == gs {
		fm += gm
	} else {
		fm -= gm
	...
	//fmt.Println(" fs/fm/fe-2/t", fs, fm, fe-2, trunc)
...
func fmul64(f, g uint64) uint64 {
	...
	lo, hi := mullu(fm, gm)
	//fmt.Println("fmul64 hi/lo", hi, lo, "fm/gm", fm, gm)
	shift := mantbits64 - 1
	trunc := lo & (1<<shift - 1)
	mant := hi<<(64-shift) | lo>>shift
	//fmt.Println("fmul64", fs, gs, mant, fe, ge, trunc)
